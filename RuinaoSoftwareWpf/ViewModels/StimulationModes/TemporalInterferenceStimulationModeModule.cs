namespace RuinaoSoftwareWpf;

public sealed class TemporalInterferenceStimulationModeModule
    : StimulationModeModuleBase<TiControlViewModel>
{
    public TemporalInterferenceStimulationModeModule(TiControlViewModel viewModel)
        : base(StimulationModeCodes.TemporalInterference, viewModel)
    {
        viewModel.BackRequested += (_, _) => RaiseBackRequested();
        viewModel.HardwareOperationCompleted += (_, result) =>
            RaiseHardwareOperationCompleted(result);
        viewModel.PrescriptionRequested += (_, eventArgs) =>
            RaisePrescriptionRequested(eventArgs);
    }

    public override void PrepareForActivation()
    {
        ViewModel.RestoreLastSelection();
    }

    public override string GetTargetChannelName(object? targetChannel)
    {
        return targetChannel is ChannelConfig channel ? channel.Name : string.Empty;
    }

    public override bool TryApplyPrescription(
        PrescriptionDefinition prescription,
        StimulationPrescriptionApplyScope scope,
        object? targetChannel,
        out string error)
    {
        if (!ValidatePrescriptionMode(prescription, out error))
        {
            return false;
        }

        if (scope == StimulationPrescriptionApplyScope.AllChannels)
        {
            ViewModel.ApplyPrescription(prescription);
            return true;
        }

        if (targetChannel is ChannelConfig channel)
        {
            ViewModel.ApplyPrescription(prescription, channel);
            return true;
        }

        error = "未找到 TI 目标通道。";
        return false;
    }
}
