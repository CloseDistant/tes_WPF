...
/// tACS 控制页面 ViewModel。
///
/// 负责维护独立tACS的16个通道、当前选中行，以及开始、停止和急停等页面级命令。
/// Shell 只负责展示该页面，不再持有 tACS 控制页的业务状态。
/// </summary>
public sealed class TacsControlViewModel : ObservableObject
{
    ...
    private readonly ITacsWaveformPreviewFactory waveformPreviewFactory;
    ...
    private readonly Dictionary<ChannelConfig, AlternatingCurrentWaveformRuntime> activeWaveforms = [];
    ...
    public TacsControlViewModel(
        ...
        ITacsWaveformPreviewFactory? waveformPreviewFactory = null)
    {
        ...
        this.waveformPreviewFactory = waveformPreviewFactory ?? new TacsWaveformPreviewFactory();
        this.logger = logger;
        ...
        Channels = new ObservableCollection<ChannelConfig>(
            Enumerable.Range(1, 16).Select(channelNumber => CreateChannel(channelNumber, accent)));
        ChannelPairs = new ObservableCollection<StimulationChannelPair>(
            Enumerable.Range(0, 8).Select(pairIndex => new StimulationChannelPair(
                pairIndex + 1,
                Channels[pairIndex * 2],
                Channels[pairIndex * 2 + 1])));
        Groups = new ObservableCollection<TiGroup>(ChannelPairs.Select(CreateDisplayRow));

        SelectChannelCommand = new RelayCommand(parameter =>
        {
            if (parameter is ChannelConfig channel && Channels.Contains(channel))
            {
                SelectedGroup = Groups.First(group => group.Channels.Contains(channel));
    ...
    /// <summary>
    /// 硬件操作完成事件。
    /// MainViewModel订阅它，用来刷新底部状态栏，避免tACS页面直接持有Shell。
    /// </summary>
    ...
    private ObservableCollection<TiGroup> Groups { get; }

    public ICommand SelectChannelCommand { get; }

    public ICommand StartCommand { get; }

    public ICommand SynchronizedStartCommand => StartCommand;
    ...
    public IReadOnlyList<ChannelConfig> SelectedChannels =>
        SelectedGroup is null ? Array.Empty<ChannelConfig>() : SelectedGroup.Channels;
    ...
    /// <summary>
    /// 回到 tACS 控制页时恢复上一次选择；如果没有历史选择，则默认选中第一组。
    /// </summary>
    public void RestoreLastSelection()
    {
        if (lastSelectedGroup is not null && Groups.Contains(lastSelectedGroup))
        {
            ...
            SelectedChannel ??= lastSelectedGroup.Channels.FirstOrDefault();
            return;
        }

        SelectedGroup = Groups.FirstOrDefault();
        SelectedChannel = SelectedGroup?.Channels.FirstOrDefault();
    }
        ...
        IEnumerable<ChannelConfig> targetChannels)
    {
        ...
        var current = prescription.TacsPeakCurrentMilliampere.ToString("0.000", System.Globalization.CultureInfo.InvariantCulture);
        var durationSeconds = prescription.TacsTotalDurationSeconds
            .ToString("0.0", System.Globalization.CultureInfo.InvariantCulture);
        var rampUpSeconds = prescription.TacsRampUpSeconds
            .ToString("0.0", System.Globalization.CultureInfo.InvariantCulture);
        var rampDownSeconds = prescription.TacsRampDownSeconds
            .ToString("0.0", System.Globalization.CultureInfo.InvariantCulture);
        var frequency = prescription.TacsFrequencyHz.ToString(System.Globalization.CultureInfo.InvariantCulture);
    ...
    private static ChannelConfig CreateChannel(int channelNumber, Brush accent) => new()
    {
        Name = $"CH {channelNumber}",
        Anode = $"E{channelNumber * 2 - 1}",
        Cathode = $"E{channelNumber * 2}",
        CurrentMA = TacsParameterRules.DefaultPeakCurrentMilliampere,
        RampUpS = TacsParameterRules.DefaultRampUpSeconds,
        RampDownS = TacsParameterRules.DefaultRampDownSeconds,
        DurationS = TacsParameterRules.DefaultTotalDurationSeconds,
        IntervalS = "0.0",
        SingleDurationS = "0.0",
        ContinuousTimingPlaceholder = "-",
        FrequencyHz = TacsParameterRules.DefaultFrequencyHz,
        StimulationMode = "连续",
        Polarity = "不掉转",
        AccentBrush = accent,
    };

    private static TiGroup CreateDisplayRow(StimulationChannelPair pair)
    {
        var group = new TiGroup { Title = $"tACS {pair.PairNumber}" };
        group.Channels.Add(pair.FirstChannel);
        group.Channels.Add(pair.SecondChannel);
        return group;
    }
    ...
    private ICommand CreateHardwareCommand(Func<object?, Task> execute, Action<Exception>? onError = null)
    {
        return new AsyncRelayCommand(
            ...
            onError: onError ?? (ex => logger.Error("tACS 控制命令执行失败", ex)));
    }
    ...
    private void HandleStopFailure(ChannelConfig channel, Exception exception)
    {
        ...
        logger.Error($"tACS 刺激停止失败：{channel.Name}", exception);
        toastService.ShowError(
            "刺激停止失败",
        ...
        out string error)
    {
        foreach (var channel in channels)
        {
            if (!TacsParameters.TryCreate(channel, out _, out error))
            {
                return false;
            }
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            logger.Debug("PROTO START channel skipped: channel is not in selected tACS row");
            return;
        }
        ...
        if (!TacsParameters.TryCreate(channel, out _, out var error))
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var result = await stimulationEngine.StartTacsGroupAsync(
                executionGroup,
            ...
            foreach (var channel in channels)
            {
                ...
                activeWaveforms[channel] = new AlternatingCurrentWaveformRuntime(
                    startTimestamp,
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            channel.IsStateUnknown = false;
            RefreshStartCommandStates();
            logger.Info($"tACS通道手动停止成功：{channel.Name}");
            HardwareOperationCompleted?.Invoke(this, result);
        }
        catch
        {
    ...
    private async Task EmergencyStopAllChannelsAsync()
    {
        ...
        {
            logger.Debug("PROTO EMERGENCY skipped: no active tACS channels");
            return;
        }

        var runningGroup = CreateExecutionGroup("tACS 运行通道", runningChannels);
        var result = await stimulationEngine.EmergencyStopTacsGroupAsync(runningGroup, "用户点击急停");
    ...
    private async Task CompleteChannelAsync(ChannelConfig channel)
    {
        ...
        {
            logger.Error($"tACS 通道 {channel.Name} 完成记录失败", ex);
        }
    }
        ...
        IEnumerable<ChannelConfig> channels)
    {
        ...
        foreach (var channel in channels)
        {
            if (!TacsParameters.TryCreate(channel, out var parameters, out var error)
                || parameters is null)
            {
    ...
    private sealed record AlternatingCurrentWaveformRuntime(
        long StartTimestamp,
