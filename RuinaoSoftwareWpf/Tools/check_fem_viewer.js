...
if (!/ActivateViewModelAsync[\s\S]*?EnsureBundledSampleLoadedAsync[\s\S]*?viewModel\.Is3D[\s\S]*?NavigateCurrentResultAsync/.test(wpfViewCode)) throw new Error('FEM activation does not load the bundled result before restoring the 3D view.');
if (!/var viewer = slicePanViewer;[\s\S]*?slicePanViewer = null;[\s\S]*?viewer\.ReleaseMouseCapture\(\)/.test(wpfViewCode)) throw new Error('2D pan cleanup is still vulnerable to reentrant mouse-capture events.');
